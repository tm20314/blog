...
`content` は装飾を使わず、手早く本文だけを書く場合や既存記事との互換用に使えます。装飾を文章の途中へ入れる記事では `blocks` を記事本文として使い、`richText`（本文）と各装飾を好きな順番で追加・並べ替えます。たとえば「本文 → 囲み → 本文 → 商品紹介」と並べれば、その順番で表示されます。

`blocks` 内に `richText` が1つでもある場合は、並び順を優先するため `content` は表示しません。`richText` がない従来の記事だけは、これまでどおり `content` の後に装飾が表示されます。
...
3. 装飾なしなら `かんたん本文`、文章の途中に装飾を入れるなら `記事本文` に「本文」と装飾を順番に追加します。
